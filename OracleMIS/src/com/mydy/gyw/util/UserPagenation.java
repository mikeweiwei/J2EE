...
public class UserPagenation {
	 ...
	 */
	public UserPagenation(Connection con, int pageCode, int pageSize, int num) {
		this.con = con;
	...
	private int countRows(){
		String sql = "select count(1) from gyw_user";
	...
	public List<Map<String, Object>> showPage() {
		...
		String sql = "select * from ( select  UNAME, UPASSWORD, PHONE, rownum RNO from gyw_user) where RNO between ? and ?";
		//sql = sqlString;
		...
		try {
			...
			while (rs.next()) {
				...
				u1.put("uname", rs.getString("uname"));
				u1.put("password", "********");
				u1.put("phone", rs.getString("phone"));
